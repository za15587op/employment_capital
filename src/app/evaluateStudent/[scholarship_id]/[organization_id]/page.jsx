...
export default function AdminPage() {
  ...
  const fetchStudentData = async (scholarship_id, organization_id) => {
    try {
      ...
      console.log("Fetched student data:", data); // Log student data for debugging
      setStudentData(data);
      setSuccess("ข้อมูลถูกโหลดสำเร็จ!");

      // Fetch matching percentages for each student
    ...
    try {
      const percentages = await Promise.all(
        students.map(async (student) => {
          console.log("Fetching requirements for student:", student);
          ...
          const response = await fetch('/api/matching', {
            ...
            body: JSON.stringify({ scholarshipId: student.scholarship_id }),
          });
      ...
      console.log("Updated match percentages:", percentageMap);
  ...
  {matchPercentage[student.student_id] !== undefined 
    ? `${matchPercentage[student.student_id].toFixed(2)}%`  // ปรับเป็นทศนิยม 2 ตำแหน่ง
    : "กำลังโหลด..."}
</td>
