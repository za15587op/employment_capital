...
async function handleFileUpload(formData) {
  ...
  if (file) {
    const fileUrl = await uploadFileToHostinger(file, student_id, scholarship_id, scholarships);
    return fileUrl; // ส่งคืน URL ของไฟล์ที่ถูกอัปโหลด
  }
...
export async function POST(req) {
  try {
    ...
    // จัดการการอัปโหลดไฟล์และได้ URL ของไฟล์
    ...
    // ตรวจสอบว่ามีการอัปโหลดไฟล์หรือไม่ ถ้ามี ให้ใช้ URL ของไฟล์ที่อัปโหลดจริง ๆ
    const fileOrWorksUrl = fileUrl || related_works;

    // เรียกใช้การสร้าง record ในฐานข้อมูล
    const registrationId = await ScholarshipRegistrations.create({
      student_id,
      scholarship_id,
      file_path: fileOrWorksUrl, // ใช้ URL ของไฟล์แทนที่ถ้ามีการอัปโหลดไฟล์
      student_status,
    });
...
export async function DELETE(req) {
  try {
    ...
    await DateTimeAvailable.delete({ where: { registrationId: regist_id } });
    await ScholarshipRegistrations.delete({ where: { id: regist_id } });
